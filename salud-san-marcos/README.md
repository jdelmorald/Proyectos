# Sistema de Documentos Operativos — Salud San Marcos

Sistema web para generar y controlar todos los documentos operativos de Salud San Marcos,
...
4. La primera vez tardará varios minutos. Al terminar, se crea automáticamente un
   acceso directo llamado **"Salud San Marcos"** en el Escritorio, y el sistema queda
   corriendo de inmediato.
5. De ahí en adelante, para usar el sistema: doble clic en el acceso directo
   **"Salud San Marcos"** del Escritorio (abre el navegador en `http://localhost:4002`).
...
de `http://localhost:4002` deben entrar a `http://<IP-de-esa-PC>:4002` (la IP se ve
...
4. La primera vez tardará varios minutos. Cuando termine, deje esa ventana abierta y
   abra su navegador en: **http://localhost:4002**
...
npm install
npm run build
npm start               # todo disponible en http://localhost:4002 (o el PORT configurado)
```
