...
export default function ComprobanteTemplate({ empresa, config, doc }: { empresa: any; config: DocumentTypeConfig; doc: any }) {
  const contraparteLabel = config.contraparte === 'cliente' ? 'Recibido de' : config.contraparte === 'proveedor' ? 'Pagado a' : null;
  const monto: number | undefined = typeof doc.datos.monto === 'number' ? doc.datos.monto : undefined;
  ...
  return (
    <div className="text-slate-900 text-sm">
      ...
      {monto !== undefined && (
        <div className="border border-slate-800 rounded-md p-4 mb-4 flex justify-between items-center">
          <div>
            <div className="text-xs uppercase text-slate-400 font-semibold">Monto en letras</div>
            <div className="font-medium">{numeroALetras(monto, empresa.moneda === 'Bs.' ? 'BOLÍVARES' : empresa.moneda)}</div>
          </div>
          <div className="text-2xl font-bold whitespace-nowrap ml-4">
            {empresa.moneda} {monto.toFixed(2)}
          </div>
        </div>
      )}
      ...
      {config.tieneFirmantes && doc.firmantes.length > 0 ? (
        ...
        <div className="mt-16 text-center">
          <div className="border-t border-slate-800 pt-1 inline-block px-16">Recibí conforme</div>
        </div>
      )}
