...
const email = process.env.SEED_ADMIN_EMAIL || 'admin@indelderca.com';
...
db.prepare(
  `UPDATE empresa SET nombre='Indelderca', moneda=COALESCE(NULLIF(moneda,''),'Bs.'), iva_porcentaje_default=COALESCE(iva_porcentaje_default,16), logo_base64=COALESCE(logo_base64, ?) WHERE id=1`
).run(LOGO_BASE64);
