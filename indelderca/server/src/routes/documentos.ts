...
const SELECT_BASE = `
  SELECT d.*, c.nombre AS contacto_nombre, c.rif_cedula AS contacto_rif, r.numero AS referencia_numero
  FROM documentos d
  LEFT JOIN contactos c ON c.id = d.contacto_id
...
documentosRouter.post('/', (req, res) => {
  ...
  if ((config.contraparte === 'cliente' || config.contraparte === 'proveedor') && !d.contactoId) {
    return res.status(400).json({ error: `Debe seleccionar un(a) ${config.contraparte}` });
  }
