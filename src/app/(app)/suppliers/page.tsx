          ...
          {suppliers.map((s) => {
            ...
            return (
              <Link
                key={s.id}
                href={`/suppliers/${s.id}`}
                className="bg-surface border border-line rounded-2xl p-4 hover:border-accent/50 transition-colors flex flex-col gap-2"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <p className="font-medium text-ink truncate">{s.legalName}</p>
                    {s.tradeName && (
                      <p className="text-xs text-ink-soft truncate">{s.tradeName}</p>
                    )}
                  </div>
